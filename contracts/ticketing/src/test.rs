...
#[test]
fn owner_of_returns_ticket_owner() {
    let (env, client, _token, _token_asset, _admin, organizer) = setup();
    make_event(&env, &client, &organizer, 1);
    let buyer = Address::generate(&env);
    let ticket_id = client.issue_ticket(
...
#[test]
fn owner_of_fails_for_invalid_ticket() {
    let (_env, client, _token, _token_asset, _admin, _organizer) = setup();
    let result = client.try_owner_of(&9999);
    assert_eq!(result, Err(Ok(Error::TicketNotFound)));
}

#[test]
fn list_tickets_with_pagination() {
    let (env, client, _token, _token_asset, _admin, organizer) = setup();
    make_event(&env, &client, &organizer, 1);
    let buyer = Address::generate(&env);
    ...
    for i in 0..5 {
        client.issue_ticket(
            ...
            &buyer,
            &String::from_str(&env, "GA"),
            &String::from_str(&env, &format!(&env, "seat_{}", i)),
            &5_000i128,
        );
    }

    // List first 2 tickets
    let tickets = client.list_tickets(&1, &0, &2);
    assert_eq!(tickets.len(), 2);

    // List next 2 tickets (skip first 2)
    let tickets = client.list_tickets(&1, &2, &2);
    assert_eq!(tickets.len(), 2);

    // List remaining (skip first 4)
    let tickets = client.list_tickets(&1, &4, &10);
    assert_eq!(tickets.len(), 1);

    // List beyond range
    let tickets = client.list_tickets(&1, &10, &10);
    assert_eq!(tickets.len(), 0);
}

#[test]
fn list_tickets_rejects_invalid_limit() {
    let (_env, client, _token, _token_asset, _admin, _organizer) = setup();
    let result = client.try_list_tickets(&1, &0, &0);
    assert_eq!(result, Err(Ok(Error::BatchTooLarge)));

    let result = client.try_list_tickets(&1, &0, &(MAX_BATCH_SIZE + 1));
    assert_eq!(result, Err(Ok(Error::BatchTooLarge)));
}

#[test]
fn list_tickets_for_nonexistent_event() {
    let (_env, client, _token, _token_asset, _admin, _organizer) = setup();
    let result = client.try_list_tickets(&9999, &0, &10);
    assert_eq!(result, Err(Ok(Error::EventNotFound)));
}

#[test]
fn upgrade_ticket_applies_price_difference() {
    let (env, client, token, _token_asset, _admin, organizer) = setup();
    make_event(&env, &client, &organizer, 1);
    let buyer = Address::generate(&env);
    
    // Mint tokens to buyer for upgrade payment
    ...
    // Upgrade from 5000 to 7000 (difference: 2000)
    client.upgrade_ticket(
        &buyer,
        &ticket_id,
        &String::from_str(&env, "VIP"),
        &7_000i128,
    ...
    let ticket = client.verify_ticket(&ticket_id);
    assert_eq!(ticket.tier, String::from_str(&env, "VIP"));
    assert_eq!(ticket.original_price, 7_000i128);
    assert_eq!(ticket.status, TicketStatus::Valid);
    assert_eq!(token.balance(&organizer), organizer_before + 2_000i128);
}

#[test]
fn upgrade_ticket_rejects_lower_price() {
    let (env, client, token, _token_asset, _admin, organizer) = setup();
    make_event(&env, &client, &organizer, 1);
    let buyer = Address::generate(&env);
    ...
    let result = client.try_upgrade_ticket(
        &buyer,
        &ticket_id,
        &String::from_str(&env, "GA"),
        &3_000i128,
...
#[test]
fn upgrade_ticket_not_owner_rejects() {
    let (env, client, token, _token_asset, _admin, organizer) = setup();
    make_event(&env, &client, &organizer, 1);
    let buyer = Address::generate(&env);
    let impostor = Address::generate(&env);
